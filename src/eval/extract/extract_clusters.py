...
if __name__ == "__main__":
    ...
    selectivity = [
        ("faces", "vision", "viridis"), 
        # ("scenes", "vision", "viridis"), 
        # ("bodies", "vision", "viridis"), 
        # ("vwfa", "vision", "inferno"), 
        # ("objects", "vision", "viridis"), 
        # ("speech", "audio", "viridis"), 
        # ("vocals", "audio", "viridis"), 
        # ("fedorenko_words_nonwords", "language", "viridis"),
        # ("theory_of_mind_text", "cognitive", "viridis"),
    ...
    top_k_pct = 1 # top k percent of units to keep

    island_morans_I_results = {}

    for category, modality, cmap in selectivity:

        print(f"> Visualizing selectivity for category: {category}, modality: {modality}")

        fig, ax = plt.subplots(figsize=(12, 6), dpi=300)
        filepath = f"{dirpath}/{category}/{category}_even_selectivity_stats.pkl"
        ...
        writepath = f"{dirpath}/{category}/cluster_selectivity_{category}_t_values_top{top_k_pct}_island=largest_mask.pkl"
        ...
        p_values[~mask_labeled.astype(bool)] = np.inf
        ...
        if filter_out_non_significant:
            t_values[~mask_labeled.astype(bool)] = np.nan
        ...
        sns.heatmap(t_values, cbar=True, cmap=cmap, ax=ax, linewidths=0, linecolor=None)

        if overlay and not filter_out_non_significant:
            gray_overlay = np.where(~mask_labeled.astype(bool), 1.0, np.nan)
        ...
        if top_k_pct <= 0 or top_k_pct > 100:
            savepath = f"{dirpath}/{category}/cluster_selectivity_{category}_t_values_p{p_value_threshold}_anatomical={anatomical_constraint}_filternonsig={filter_out_non_significant}"
        else:
            savepath = f"{dirpath}/{category}/cluster_selectivity_{category}_t_values_top{top_k_pct}_anatomical={anatomical_constraint}_filternonsig={filter_out_non_significant}"
